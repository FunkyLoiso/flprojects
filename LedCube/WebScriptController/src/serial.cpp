/*  Serial port object for use with Qt
 *	based on
...
int Serial::Open(const QString& _name)
{
	txCount = rxCount = 0;
    QByteArray tempArray = _name.toLatin1();
	const char *name = tempArray.constData();
	Close();
#if defined(LINUX)
	//struct serial_struct kernel_serial_settings;
	//int bits;
	//port_fd = open(name, O_RDWR | O_NOCTTY | O_NONBLOCK);
	//if (port_fd < 0) {
	//	if (errno == EACCES) {
	...
	if (p && sscanf(p + 3, "%d", &port_num) == 1) {
		printf("port_num = %d\n", port_num);
		sprintf_s(name_createfile, sizeof(name_createfile), "\\\\.\\COM%d", port_num);
		sprintf_s(name_commconfig, sizeof(name_commconfig), "COM%d", port_num);
	} else {
		sprintf_s(name_createfile, sizeof(name_createfile), "%s", name);
		sprintf_s(name_commconfig, sizeof(name_commconfig), "%s", name);
	}
	...
	if (port_handle == INVALID_HANDLE_VALUE) {
		//win32_err(buf);
		//error_msg = QString("Unable to open %1, %2").arg(name).arg(buf);
		return -1;
	}
	len = sizeof(COMMCONFIG);
	if (!GetCommConfig(port_handle, &port_cfg, &len)) {
		...
		//win32_err(buf);
		//error_msg = QString("Unable to read communication config on %1, %2").arg(name).arg(buf);
		return -1;
	}
	if (!got_default_cfg) {
	...
	if (!SetCommConfig(port_handle, &port_cfg, sizeof(COMMCONFIG))) {
		CloseHandle(port_handle);
		//win32_err(buf);
		//error_msg = QString("Unable to write communication config to %1, %2").arg(name).arg(buf);
		return -1;
	}
	if (!EscapeCommFunction(port_handle, CLRDTR | CLRRTS)) {
		CloseHandle(port_handle);
		//win32_err(buf);
		//error_msg = QString("Unable to control serial port signals on %1, %2").arg(name).arg(buf);
		return -1;
	}
	...
	if (!SetCommTimeouts(port_handle, &timeouts)) {
		CloseHandle(port_handle);
		//win32_err(buf);
		//error_msg = QString("Unable to write timeout settings to %1, %2").arg(name).arg(buf);
		return -1;
	}
#endif
	port_name = name;
...
QString Serial::get_name(void)
{
	if (!port_is_open) return QString();
	return port_name;
}
...
QString Serial::error_message(void)
{
	return error_msg;
}
...
int Serial::Set_baud(const QString& baud_str)
{
	unsigned long b;
	//if (!baud_str.ToULong(&b)) return -1;
	bool ok = false;
	b = baud_str.toULong(&ok);
	if(!ok) return -1;
...
#if defined(MACOSX)
static void macos_ports(io_iterator_t  * PortIterator, QStringList& list)
{
...
QStringList Serial::port_list()
{
	...
	while ((f = readdir(dir)) != NULL) {
		...
		sprintf_s(s, sizeof(s), "/dev/%s", f->d_name);
