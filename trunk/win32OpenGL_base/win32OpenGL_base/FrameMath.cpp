...
void gltRotateFrameLocalY(GLTFrame *pFrame, GLfloat fAngle)
    ...
    //gltRotationMatrix((float)gltDegToRad(fAngle), 0.0f, 1.0f, 0.0f, mRotation);//Нафига эта строчка...?
...
/*Вращает актёра вокруг глобальной оси y. Написал для вращения камеры, чтобы она всегда смотрела параллельно земле*/
void gltRotateFrameWorldY(GLTFrame *pFrame, GLfloat fAngle)
{
	GLTMatrix mRotation;
	GLTVector3 vNewVect;
    
    gltRotationMatrix((float)fAngle, 0.0f, 1.0f, 0.0f, mRotation);

	//GLTVector3 vNewForward;

    gltRotateVector(pFrame->vForward, mRotation, vNewVect);
    memcpy(pFrame->vForward, vNewVect, sizeof(GLTVector3));

	gltRotateVector(pFrame->vUp, mRotation, vNewVect);
	memcpy(pFrame->vUp, vNewVect, sizeof(GLTVector3));
}