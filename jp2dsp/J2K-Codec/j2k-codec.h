...
#ifdef __cplusplus
 J2K_CODEC_API(int) J2K_Decode(void *image, unsigned char **buffer, int *size, char *options="", int *pitch=NULL); 
#else
 J2K_CODEC_API(int) J2K_Decode(void *image, unsigned char **currentBuffer, int *size, char *options, int *pitch); 
#endif
...
class J2K_Image
{
 protected:
  ...
  void reset(){ image=0; width=height=components=0; hTiles=vTiles=0; fileType=0; precision=0; errStr=""; buffer=0; size=0; pitch=0; }
  int _open()
  {
 ...
 public:
   ...
   int width, height, components, hTiles, vTiles, fileType, precision; const char *errStr;
	...
	for(int i=0; i<TotalFrames; i++)
	{
		...
		if(J2K_GetResolutionDimensionsByStr(image, options, &tileWidth, &tileHeight) == J2KERR_SUCCESS)
		{
			...
			if(frame[i])
			{
				...
				strcpy_s(new_options, 128, options); strcat_s(new_options, 128, ",bpp=4"); // Override user's bpp, because frames have always bpp = 4
