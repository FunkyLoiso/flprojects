...
//	Крутит вектор по ссылке.. Вроде быстрее некуда)
void gltTransformPoint(GLTVector3 *Point, const GLTMatrix Matrix)
{
	GLTVector3 temp = {(*Point)[0], (*Point)[1], (*Point)[2]};
	(*Point)[0] = Matrix[0] * temp[0] + Matrix[4] * temp[1] + Matrix[8] *  temp[2] + Matrix[12];
    (*Point)[1] = Matrix[1] * temp[0] + Matrix[5] * temp[1] + Matrix[9] *  temp[2] + Matrix[13];
    (*Point)[2] = Matrix[2] * temp[0] + Matrix[6] * temp[1] + Matrix[10] * temp[2] + Matrix[14]; 
}
